<?php
	...
	class Ticket extends MetadataModel {
		...
		function init() {
			...
			if (! $this->id ) {
				...
				$this->status = '';
				$this->replies = 0;
				$this->start = '0000-00-00 00:00:00';
		...
		function save() {
			...
			try {
				$sql = "INSERT INTO banana_ticket (id, user_id, project_id, client_id, subject, details, attachments, status, replies, start, due, created, modified)
						VALUES (:id, :user_id, :project_id, :client_id, :subject, :details, :attachments, :status, :replies, :start, :due, :created, :modified)
						ON DUPLICATE KEY UPDATE project_id = :project_id, client_id = :client_id, subject = :subject, details = :details, attachments = :attachments, status = :status, start = :start, due = :due, modified = :modified";
	...
	class Tickets {
		...
		static function get($id) {
			...
			$dbh = $site->getDatabase();
			try {
				$sql = "SELECT id, user_id, project_id, client_id, subject, details, attachments, status, replies, start, due, created, modified FROM banana_ticket WHERE id = :id";
				$stmt = $dbh->prepare($sql);
		...
		static function all($page = 1, $show = 30, $sort = 'asc', $by = 'id') {
			...
			$offset = $show * ($page - 1);
			# Sanity checks
			$by = in_array($by, array('id', 'user_id', 'project_id', 'client_id', 'subject', 'details', 'attachments', 'status', 'replies', 'start', 'due', 'created', 'modified') ) ? $by : false;
			...
			try {
				$sql = "SELECT id, user_id, project_id, client_id, subject, details, attachments, status, replies, start, due, created, modified FROM banana_ticket ORDER BY {$by} {$sort} LIMIT {$offset},{$show}";
				$stmt = $dbh->prepare($sql);
				$stmt->setFetchMode(PDO::FETCH_CLASS, 'Ticket');
				$stmt->execute();
				$ret = $stmt->fetchAll();
		...
		static function rawWhere($conditions, $page = 1, $show = 30, $sort = 'asc', $by = 'id') {
			...
			try {
				$sql = "SELECT id, user_id, project_id, client_id, subject, details, attachments, status, replies, start, due, created, modified FROM banana_ticket WHERE {$conditions} ORDER BY {$by} {$sort} LIMIT {$offset},{$show}";
				$stmt = $dbh->prepare($sql);
				$stmt->setFetchMode(PDO::FETCH_CLASS, 'Ticket');
				$stmt->execute();
				$ret = $stmt->fetchAll();
